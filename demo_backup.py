...
class Environment(object):
    ...
    def fetch_question(self, id = None):
        ...
        '''
        if id is None:
            id = self.cur_batch_id
        if id > self.data_obj.n_batches:
            assert 'question index out of range'
            self.reset_file()
        train_batch_dict = self.data_obj.get_batch(id, self.data_obj.train_data, self.data_obj.train_data_map, self.data_obj.batch_size_types)
        self.cur_batch_id += 1
        ...
        return input_questions, entities_in_questions, types_in_questions, relations_in_questions, ints_in_questions, id
    ...
    def step(self,id,programs_batch):
        ...
        return self._step(id,[[programs_batch]])

    def _step(self,id,programs_batch):
        '''
